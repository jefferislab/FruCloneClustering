...
input_files=dir([input_dir,'*-tubed.PIC']);
for i=1:length(input_files)
	...
	output_files=dir([output_dir,'*_tubed.mat']);
	...
	lock_files=dir([output_dir,'*-in_progress.mat']);
	...
	if processFile==1
		...
		save([output_dir,input_files(i).name,'-in_progress.mat'],'processFile');

		% read in image
		x=readpic([tubed_dir,input_files(i).name]);
		...
		% delete lockfile
		delete([output_dir,input_files(i).name,'-in_progress.mat'])
	end
